...
func (p *SendMsgHandler) HandleProcess() error {
	...
	if len(limitdiv) > 0 && config.Conf.Common.OpenCache {
...
func (p *SendMsgHandler) sendToMySQL() error {
	...
	// 同时在消息记录表中创建记录，设置初始重试次数为0
	var msgRecord = new(data.MsgRecord)
	msgRecord.Subject = p.Req.Subject
	msgRecord.MsgId = msgID
	msgRecord.TemplateID = p.Req.TemplateID
	msgRecord.To = p.Req.To
	msgRecord.Status = int(data.TASK_STATUS_PENDING)
	msgRecord.RetryCount = 0

	// 将模板数据设置到消息记录中
	msgRecord.TemplateData = string(td)

	// 获取模板信息以设置渠道和来源ID
	tpl, tplErr := dt.GetMsgTemplate(context.Background(), p.Req.TemplateID)
	if tplErr == nil && tpl != nil {
		msgRecord.Channel = tpl.Channel
		msgRecord.SourceID = tpl.SourceID
	}
